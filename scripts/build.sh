...
echo -e "${BLUE}🦉 Cyberwritr Build Script${NC}"
echo "================================"
...
uv run pyinstaller cyberwritr.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="cyberwritr-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/cyberwritr.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/cyberwritr.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/cyberwritr" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/cyberwritr" "$RELEASE_DIR/$BINARY_NAME"
