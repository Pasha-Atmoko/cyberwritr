"""Tests for cyberwritr.config.loader: JSON overrides, alias resolution, persistence."""
...
from cyberwritr.config import loader
...
_LLM_ENV_KEYS = [
    ...
    "LITELLM_BASE_URL",
    "OLLAMA_API_BASE",
    "CWRITR_REASONING_EFFORT",
    "LLM_TIMEOUT",
    "PERPLEXITY_API_KEY",
    # RuntimeSettings
    "CWRITR_IMAGE",
    "CWRITR_RUNTIME_BACKEND",
    "CWRITR_MAX_LOCAL_COPY_MB",
    # TelemetrySettings
    "CWRITR_TELEMETRY",
]
...
def test_read_json_overrides_maps_to_nested_settings(tmp_path: Path) -> None:
    path = tmp_path / "cli-config.json"
    path.write_text(
        json.dumps({"env": {"CWRITR_LLM": "my-model", "PERPLEXITY_API_KEY": "pk"}}),
        encoding="utf-8",
    )
    assert loader._read_json_overrides(path) == {
        "llm": {"model": "my-model"},
...
def test_read_json_overrides_skips_keys_already_in_environ(
    ...
    monkeypatch.setenv("CWRITR_LLM", "from-env")
    path = tmp_path / "cli-config.json"
    path.write_text(json.dumps({"env": {"CWRITR_LLM": "from-file"}}), encoding="utf-8")
...
def test_apply_override_and_load_settings_round_trip(tmp_path: Path) -> None:
    path = tmp_path / "cli-config.json"
    path.write_text(
        json.dumps({"env": {"CWRITR_LLM": "round-trip-model", "PERPLEXITY_API_KEY": "pk"}}),
        encoding="utf-8",
    )
...
def test_apply_config_override_invalidates_cache(tmp_path: Path) -> None:
    ...
    first.write_text(json.dumps({"env": {"CWRITR_LLM": "first-model"}}), encoding="utf-8")
    second = tmp_path / "second.json"
    second.write_text(json.dumps({"env": {"CWRITR_LLM": "second-model"}}), encoding="utf-8")
...
def test_persist_current_writes_env_block(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CWRITR_LLM", "persisted-model")
    ...
    assert json.loads(target.read_text(encoding="utf-8")) == {
        "env": {"CWRITR_LLM": "persisted-model"}
    }
