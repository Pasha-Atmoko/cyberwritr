"""Tests for graceful handling of persistent RateLimitError in run_cyberwritr_scan."""
...
import cyberwritr.tools.notes.tools as notes_tools
import cyberwritr.tools.todo.tools as todo_tools
from cyberwritr.core import runner
...
async def test_persistent_rate_limit_stops_gracefully(
    ...
    monkeypatch.setattr(runner, "build_cyberwritr_agent", lambda **_kwargs: object())
    ...
    with caplog.at_level(logging.WARNING):
        result = await runner.run_cyberwritr_scan(
    ...
    assert "cyberwritr --resume scan-test" in caplog.text
