...
from cyberwritr.config.models import _configure_litellm_compatibility
from cyberwritr.report.state import litellm_cost_callback


def test_streaming_logging_stays_enabled_for_cost_callback() -> None:
    with (
        patch.object(litellm, "disable_streaming_logging", new=True),
        patch("cyberwritr.config.models._register_litellm_cost_callback") as register,
    ):
...
def test_cost_callback_reads_openrouter_stream_usage_cost() -> None:
    ...
    with patch("cyberwritr.report.state.get_global_report_state", return_value=report_state):
