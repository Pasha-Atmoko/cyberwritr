...
Cyberwritr offers three scan modes to balance speed and thoroughness.

## Quick

```bash
cwritr --target ./app --scan-mode quick
```
...
```bash
cwritr --target ./app --scan-mode standard
```
...
```bash
cwritr --target ./app --scan-mode deep
```
