---
title: "CLI Reference"
description: "Command-line options for Cyberwritr"
---

## Basic Usage

```bash
cwritr --target <target> [options]
```
...
<ParamField path="--mount" type="string">
  ...
  Cyberwritr copies local `--target` directories into the sandbox one file at a time, which stalls on very large trees. When a local target exceeds the copy limit (see `CWRITR_MAX_LOCAL_COPY_MB`, default 1024 MB) Cyberwritr exits early and asks you to re-run with `--mount`.
...
<ParamField path="--config" type="string">
  Path to a custom config file (JSON) to use instead of `~/.cyberwritr/cli-config.json`.
</ParamField>
...
<ParamField path="--max-budget-usd" type="number">
  ...
  **Limitations**
  ...
  - For LiteLLM-routed models, Cyberwritr enables streaming success callbacks to
...
```bash
# Basic scan
cwritr --target https://example.com

# Authenticated testing
cwritr --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
cwritr --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
cwritr -n --target ./ --scan-mode quick

# Force diff-scope against a specific base ref
cwritr -n --target ./ --scan-mode quick --scope-mode diff --diff-base origin/main

# Multi-target white-box testing
cwritr -t https://github.com/org/app -t https://staging.example.com

# Large local repository — bind-mount instead of copying it in
cwritr --mount ./huge-monorepo
```
