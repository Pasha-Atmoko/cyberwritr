---
title: "Local Models"
description: "Run Cyberwritr with self-hosted LLMs for privacy and air-gapped testing"
---

Running Cyberwritr with local models allows for completely offline, privacy-first security assessments. Data never leaves your machine, making this ideal for sensitive internal networks or air-gapped environments.
...
<Warning>
**Compatibility Note**: Cyberwritr relies on advanced agentic capabilities (tool use, multi-step planning, self-correction). Most local models, especially those under 70B parameters, struggle with these complex tasks.
...
2. Pull a high-performance model:
   ...
   ```
3. Configure Cyberwritr:
   ```bash
   export CWRITR_LLM="ollama/qwen3-vl"
...
```bash
export CWRITR_LLM="openai/local-model"
