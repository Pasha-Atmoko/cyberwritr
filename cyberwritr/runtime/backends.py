"""Sandbox backend registry — selected via CWRITR_RUNTIME_BACKEND (default: docker)."""
...
async def _docker_backend(
    *,
    ...
    Uses :class:`CyberwritrDockerSandboxClient` to inject NET_ADMIN /
    ...
    would call it too, but Cyberwritr manages session lifetime explicitly via
    ...
    from cyberwritr.runtime.docker_client import CyberwritrDockerSandboxClient

    client = CyberwritrDockerSandboxClient(docker.from_env())
    client.cyberwritr_bind_mounts = bind_mounts or []
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown CWRITR_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )
