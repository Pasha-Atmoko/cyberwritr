"""SARIF 2.1.0 output for Cyberwritr vulnerability reports.

Builds a GitHub code-scanning compatible SARIF document from Cyberwritr findings
...
Design notes:
  * Rules are keyed on CWE (``id = CWE-NNN``), falling back to CVE, then
    ...
    Cyberwritr output variants (``CWE-306``, ``cwe: 306``, ``306``) to the
    canonical ``CWE-NNN`` form so dedup works across runs.
  * SARIF only has three levels (error / warning / note). Cyberwritr's five
    severities collapse into them. The raw severity label and CVSS score
    survive in ``result.properties.cyberwritr`` for downstream tools that can
...
SARIF_VERSION = "2.1.0"
TOOL_NAME = "Cyberwritr"
TOOL_INFORMATION_URI = "https://cyberwritr.ai"
...
# SARIF only has three result levels; Cyberwritr's five severities collapse here.
# Original label survives in ``result.properties.cyberwritr.severity``.
_SEVERITY_TO_LEVEL = {
    "critical": "error",
    "high": "error",
    "medium": "warning",
...
def _apply_repository_context(run: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    ``automationDetails.id`` categorises the run (``cyberwritr/<owner>/<repo>``)
    so multiple Cyberwritr runs against the same repo reconcile rather than pile
    ...
    if full_name:
        run["automationDetails"] = {"id": f"cyberwritr/{full_name}"}
...
def _build_rule(rule_id: str, report: dict[str, Any]) -> dict[str, Any]:
    """Build a SARIF rule descriptor from a Cyberwritr finding."""
...
def _result_properties(
    ...
    *,
    ...
    """Cyberwritr-specific metadata for downstream consumers.

    The top-level ``security-severity`` matches GitHub code-scanning's
    expected property. Cyberwritr-specific fields are namespaced under
    ``cyberwritr`` so generic SARIF consumers don't see them by default.
    """
    properties: dict[str, Any] = {
        "security-severity": _security_severity(report),
    }
    if class_fingerprint:
        ...
        # filter alerts by it without parsing the nested cyberwritr.* tree.
        properties["cyberwritr_vuln_class_hash"] = class_fingerprint
    if is_synthetic:
        # Top-level so reviewers + downstream automation can filter
        # synthetic-anchored alerts without parsing the nested cyberwritr.*
        # tree.
        properties["synthetic_location"] = True

    cyberwritr: dict[str, Any] = {}
    for key in (
        "id",
        "severity",
        "cvss",
    ...
    if cyberwritr:
        properties["cyberwritr"] = cyberwritr
...
def _build_fixes(report: dict[str, Any]) -> list[dict[str, Any]] | None:
    ...
    Cyberwritr findings carry the suggested change inline on each code
...
def _rule_id(report: dict[str, Any]) -> str:
    ...
    CWE values are normalised from Cyberwritr output variants (``CWE-306``,
    ...
    title = _string_value(report.get("title")) or "cyberwritr-finding"
    ...
    *,
    ...
    Computed from primitives that don't depend on LLM prose stability:
      ...
      - first SARIF location's URI + startLine — these come from
        Cyberwritr's ``code_locations[].file`` and ``start_line`` which
    ...
    if uri:
        ...
        if start_line is not None:
            ...
            # line because Cyberwritr code_locations carry the SINK line,
...
def _sarif_level(severity: Any) -> str:
    """Map Cyberwritr severity labels to SARIF result levels."""
...
def _slugify(value: str) -> str:
    ...
    return slug or "cyberwritr-finding"
