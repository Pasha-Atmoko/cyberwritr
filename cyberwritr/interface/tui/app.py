...
from cyberwritr.config import load_settings
from cyberwritr.core.hooks import BudgetExceededError
from cyberwritr.core.runner import run_cyberwritr_scan
from cyberwritr.interface.tui.live_view import TuiLiveView
from cyberwritr.interface.tui.messages import send_user_message_to_agent
from cyberwritr.interface.tui.renderers import render_tool_widget
from cyberwritr.interface.tui.renderers.agent_message_renderer import AgentMessageRenderer
from cyberwritr.interface.tui.renderers.user_message_renderer import UserMessageRenderer
from cyberwritr.interface.utils import build_tui_stats_text
from cyberwritr.report.state import ReportState, set_global_report_state
from cyberwritr.runtime import session_manager
...
def get_package_version() -> str:
    try:
        return pkg_version("cyberwritr-agent")
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: CyberwritrTUIApp | None = None

    def set_app_reference(self, app: "CyberwritrTUIApp") -> None:
...
class SplashScreen(Static):  # type: ignore[misc]
    ...
    def _build_url_text(self) -> Text:
        return Text("cyberwritr.ai", style=Style(color=self.PRIMARY_GREEN, bold=True))

    def _build_welcome_text(self) -> Text:
        text = Text("Welcome to ", style=Style(color="white", bold=True))
        text.append("Cyberwritr", style=Style(color=self.PRIMARY_GREEN, bold=True))
    ...
    def _build_start_line_text(self, phase: int) -> Text:
        full_text = "Starting Cyberwritr Agent"
...
class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Cyberwritr Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class QuitScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Quit Cyberwritr?", id="quit_title"),
            Grid(
...
class CyberwritrTUIApp(App):  # type: ignore[misc]
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        from cyberwritr.core.agents import AgentCoordinator
    ...
    def _merge_renderables(renderables: list[Any]) -> Text:
        ...
        for i, item in enumerate(renderables):
            if i > 0:
                combined.append("\n")
            CyberwritrTUIApp._append_renderable(combined, item)
        return CyberwritrTUIApp._sanitize_text(combined)
    ...
    def _append_renderable(combined: Text, item: Any) -> None:
        ...
        if isinstance(item, Text):
            combined.append_text(CyberwritrTUIApp._sanitize_text(item))
        elif isinstance(item, Group):
            for j, sub in enumerate(item.renderables):
                if j > 0:
                    combined.append("\n")
                CyberwritrTUIApp._append_renderable(combined, sub)
        else:
            inner = getattr(item, "content", None) or getattr(item, "renderable", None)
            if inner is not None:
                CyberwritrTUIApp._append_renderable(combined, inner)
            else:
    ...
    def _clean_copied_text(text: str) -> str:
        ...
        for line in lines:
            ...
            if stripped in CyberwritrTUIApp._DECORATIVE_LINES:
                continue
            if stripped and all(c == "─" for c in stripped):
                continue
            out = line
            for prefix in CyberwritrTUIApp._ICON_PREFIXES:
