...
from cyberwritr.config import (
    apply_config_override,
    load_settings,
    persist_current,
)
from cyberwritr.config.models import (
    CyberwritrProvider,
    configure_sdk_model_defaults,
    is_known_openai_bare_model,
)
from cyberwritr.core.paths import run_dir_for, runtime_state_dir
from cyberwritr.interface.cli import run_cli
from cyberwritr.interface.tui import run_tui
from cyberwritr.interface.utils import (
...
from cyberwritr.report.state import get_global_report_state
from cyberwritr.report.writer import read_run_record, write_run_record
from cyberwritr.telemetry import posthog, scarf
from cyberwritr.telemetry.logging import configure_dependency_logging
...
def validate_environment() -> None:
    ...
    if not settings.llm.model:
        missing_required_vars.append("CWRITR_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "CWRITR_LLM":
                ...
                error_text.append("CWRITR_LLM", style="bold cyan")
                error_text.append(
        ...
        if missing_optional_vars:
            ...
            for var in missing_optional_vars:
                ...
                elif var == "CWRITR_REASONING_EFFORT":
                    error_text.append("• ", style="white")
                    error_text.append("CWRITR_REASONING_EFFORT", style="bold cyan")
                    error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export CWRITR_LLM='openai/gpt-5.4'\n", style="dim white")
        ...
        if missing_optional_vars:
            for var in missing_optional_vars:
                if var == "LLM_API_KEY":
                    ...
                    error_text.append(
                        "export CWRITR_REASONING_EFFORT='high'\n",
                        style="dim white",
                    )

        panel = Panel(
            error_text,
            title="[bold white]CYBERWRITR",
            title_align="left",
            border_style="red",
            padding=(1, 2),
        )
...
async def warm_up_llm() -> None:
    ...
    try:
        ...
        model = CyberwritrProvider().get_model(raw_model)
...
def get_version() -> str:
    try:
        ...
        return version("cyberwritr-agent")
    except Exception:
        return "unknown"
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Cyberwritr Multi-Agent Cybersecurity Penetration Testing Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Web application penetration test
  cyberwritr --target https://example.com

  # GitHub repository analysis
  cyberwritr --target https://github.com/user/repo
  cyberwritr --target git@github.com:user/repo.git

  # Local code analysis
  cyberwritr --target ./my-project

  # Large local repository (bind-mounted read-only instead of copied)
  cyberwritr --mount ./huge-monorepo

  # Domain penetration test
  cyberwritr --target example.com

  # IP address penetration test
  cyberwritr --target 192.168.1.42

  # Multiple targets (e.g., white-box testing with source and deployed app)
  cyberwritr --target https://github.com/user/repo --target https://example.com
  cyberwritr --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Custom instructions (inline)
  cyberwritr --target example.com --instruction "Focus on authentication vulnerabilities"

  # Custom instructions (from file)
  cyberwritr --target example.com --instruction-file ./instructions.txt
  cyberwritr --target https://app.com --instruction-file /path/to/detailed_instructions.md
        """,
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"cyberwritr {get_version()}",
    )
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to a custom config file (JSON) to use instead of ~/.cyberwritr/cli-config.json",
    )
    ...
    parser.add_argument(
        ...
        metavar="RUN_NAME",
        help=(
            "Resume a prior scan by its run name (the dir under ./cyberwritr_runs/). "
    ...
    if args.resume:
        ...
        if oversized:
            ...
            parser.error(
                ...
                "(set CWRITR_MAX_LOCAL_COPY_MB to change it). Re-run with "
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    ...
    if not scan_completed:
        ...
        resume_text.append("  ")
        resume_text.append(f"cyberwritr --resume {args.run_name}", style="#22c55e")
        panel_parts.extend(["\n", resume_text])
    ...
    console.print()
    console.print(
        "[#60a5fa]cyberwritr.ai[/]  [dim]·[/]  "
        "[#60a5fa]docs.cyberwritr.ai[/]  [dim]·[/]  "
        "[#60a5fa]discord.gg/cyberwritr-ai[/]"
    )
    console.print()
