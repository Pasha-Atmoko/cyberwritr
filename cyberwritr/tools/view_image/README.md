...
- **Implementation:** `agents.sandbox.capabilities.tools.view_image.ViewImageTool`
  (upstream `agents` SDK)
- **Wired in:** `cyberwritr/agents/factory.py` — added per-run via the SDK
  `Filesystem` capability.
- **Cyberwritr defaults:** screenshots default to
...
- **Skill:** screenshot workflow lives in `cyberwritr/skills/tooling/agent_browser.md`.
- **Recovery:** vision-not-supported model rejections are auto-recovered
  via `cyberwritr.core.sessions.strip_all_images_from_session`, invoked from
  `cyberwritr/core/execution.py`.
