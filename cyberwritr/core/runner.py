"""Top-level Cyberwritr scan runner."""
...
from cyberwritr.agents.factory import build_cyberwritr_agent, make_child_factory
...
from cyberwritr.core.execution import (
    respawn_subagents,
    run_agent_loop,
)
from cyberwritr.core.execution import (
...
from cyberwritr.core.hooks import BudgetExceededError, ReportUsageHooks
from cyberwritr.core.inputs import (
    DEFAULT_MAX_TURNS,
    build_root_task,
    build_scope_context,
    make_model_settings,
...
from cyberwritr.core.sessions import open_agent_session
from cyberwritr.runtime import session_manager
from cyberwritr.telemetry.logging import set_scan_id, setup_scan_logging
...
async def run_cyberwritr_scan(
    *,
    ...
    """Run or resume one Cyberwritr scan against a sandbox."""
    if scan_id is None:
    ...
    logger.info(
        "%s Cyberwritr scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set CWRITR_LLM env or pass model= to run_cyberwritr_scan().",
        )
    ...
    from cyberwritr.tools.notes.tools import hydrate_notes_from_disk
    from cyberwritr.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        run_config = RunConfig(
            ...
            model_provider=CyberwritrProvider(),
        ...
        root_agent = build_cyberwritr_agent(
            name="cyberwritr",
            skills=skills,
            is_root=True,
            scan_mode=scan_mode,
        ...
        logger.warning(
            ...
            "Resume with 'cyberwritr --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("Cyberwritr scan %s failed", scan_id)
        if root_id is not None:
            await coordinator.cancel_descendants(root_id)
            with contextlib.suppress(Exception):
        ...
        logger.info("Cyberwritr scan %s done", scan_id)
        teardown_logging()
